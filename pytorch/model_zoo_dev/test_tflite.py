...
def parse_args():
    parser = argparse.ArgumentParser(description="Test TensorFlow Lite model accuracy.")
...
def main():
    ...
    acc_metrics, perf_metrics = model.dataset.handler(model, args.batch_size, model.get_test_num_runs(args.batch_size), None)
    model.dataset.check_accuracy(acc_metrics)
    if args.check_performance:
        write_result_to_excel("AIO CI Perf Test.xlsx", "Results", args.model_name,  "tflite", args.precision, args.batch_size or model.default_bs, perf_metrics)
        # check_threshold("AIO CI Perf Test.xlsx", "Latency Targets", args.model_name, "tflite", args.precision, perf_metrics)
