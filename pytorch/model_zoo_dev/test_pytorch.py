...
def parse_args():
    parser = argparse.ArgumentParser(description="Test PyTorch model accuracy.")
...
def main():
    args = parse_args()
    model = pytorch_models.get_model(args.model_name, args.precision, args.import_all)
    ...
    if args.check_performance:
        write_result_to_excel("AIO CI Perf Test.xlsx", "Results", args.model_name, "pytorch", args.precision,
                              args.batch_size or model.default_bs, perf_metrics)
        # check_threshold("AIO CI Perf Test.xlsx", "Latency Targets", args.model_name, "pytorch", args.precision, perf_metrics)
